...
template <class Iterator, class UserData, class Char, class Traits, class Allocator>
const ParserSymbol* Parser<Iterator, UserData, Char, Traits, Allocator>::symbolFromToken(const ::sokira::le::CToken& token) const
{
	const ParserSymbol* symbol = NULL;
	if (token.mMatcher)
	{
		const std::vector<ptr<ParserSymbol> >& symbols = state_machine_->symbols();
		for (std::vector<ptr<ParserSymbol> >::const_iterator it = symbols.begin(); it != symbols.end(); ++it)
		{
			if (token.identifier() == (*it)->get_lexeme().c_str())
			{
				symbol = it->get();
				break;
			}
	...
	{
		symbol = state_machine_->end_symbol();
	}

	if (!symbol)
	{
		symbol = state_machine_->error_symbol();
		std::cout << "INVALID TOKEN: " << token.identifier() << std::endl;
	}

	return symbol;
}

static std::string lexemeFromToken(const sokira::le::CToken& token)
{
	return token.mMatcher ? token.mMatcher->token().UTF8String() : "";
}
...
//    lexer_.advance();
	sokira::le::CToken token = mLexer->nextToken();

    while ( parse(symbolFromToken(token), lexemeFromToken(token)) )
    {
